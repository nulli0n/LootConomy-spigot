...
import su.nightexpress.lootconomy.action.EventHelper;
import su.nightexpress.lootconomy.action.ObjectFormatter;
...
public class MythicMobsHook {
    ...
    public static void register(@NotNull ActionRegistry registry) {
        registry.registerAction(MythicMobDeathEvent.class, EventPriority.NORMAL, ACTION_TYPE);
    }
    ...
    public static final EventHelper<MythicMobDeathEvent, MythicMob> EVENT_HELPER = (plugin, event, processor) -> {
        ...
        processor.getLoot(player, mythicMob, 1).forEach(itemStack -> {
    ...
    public static final ActionType<MythicMobDeathEvent, MythicMob> ACTION_TYPE = ActionType.create("kill_mythic_mob", OBJECT_FORMATTER, EVENT_HELPER);
}
